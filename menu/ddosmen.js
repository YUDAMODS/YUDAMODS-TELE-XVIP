...
const getWebInfo = async (url) => {
  try {
    ...
    const serverInfo = response.headers['server'] || 'No server information';

    // Get content type and content length
    const contentType = response.headers['content-type'] || 'No content type information';
    const contentLength = response.headers['content-length'] || 'No content length information';

    // Get response time
    const responseTime = response.headers['x-response-time'] || 'No response time information';

    // Get DDoS protection information
    const ddosProtection = response.headers['x-ddos-protection'] || 'No DDoS protection information';

    // Get cache control information
    const cacheControl = response.headers['cache-control'] || 'No cache control information';

    // Return detailed information about the website
    return {
    ...
    throw new Error(`Unable to access ${url}: ${error.message}`);
  }
};
try {
...
bot.onText(/\.checkhost (.+)/, async (msg, match) => {
  const chatId = msg.chat.id;
  const host = match[1]; // Get the host to be checked from the user's message

  try {
    // Get detailed information about the given website
    const webInfo = await getWebInfo(`http://${host}`);

    // Format the obtained website information
    const responseData = `
🌐 *Website Information* 🌐
*Host*: ${host}
*Status Code*: ${webInfo.statusCode} ${webInfo.statusText}
*Headers*:
${webInfo.headers}
*Server*: ${webInfo.server}
*Content Type*: ${webInfo.contentType}
*Content Length*: ${webInfo.contentLength} bytes
*Response Time*: ${webInfo.responseTime}
*DDoS Protection*: ${webInfo.ddosProtection}
*Cache Control*: ${webInfo.cacheControl}
*Body*: ${webInfo.body.substring(0, 1000)}... (First snippet of the body)
`;

    // Send the website check result to the user along with a caption
    bot.sendMessage(chatId, responseData, { parse_mode: 'Markdown', caption: 'Website Check Result' });
  } catch (error) {
    // Send an error message if there is an issue with checking the website
