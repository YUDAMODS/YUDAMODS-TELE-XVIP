...
module.exports = (bot) => {
  bot.onText(/^\.tt(?:\s+(.+))?$|\.tiktok(?:\s+(.+))?$|\.tiktokdl(?:\s+(.+))?$/, async (msg, match) => {
    const chatId = msg.chat.id;
    const url = match[1];

    if (!url) {
      bot.sendMessage(chatId, 'Masukan link TikTok yang ingin Anda download, contoh: .tt https://www.tiktok.com/@username/video/1234567890123456789');
      return;
    }
